  ...
  return (
    <div className="flex items-center gap-3 p-2 bg-zinc-900 rounded-md border border-zinc-800 w-full max-w-sm">
      <div className="w-10 h-10 rounded-full overflow-hidden border border-zinc-700 flex-shrink-0">
        <img src={`https://github.com/${username}.png`} alt={dev.name} className="object-cover w-full h-full" />
      </div>
      <div className="flex flex-col gap-0.5">
        ...
        <div className="flex gap-2 text-xs text-zinc-400 flex-wrap">
          <a href={dev.github} target="_blank" className="hover:text-zinc-200 transition-colors">
            GitHub
          </a>
          <a href={dev.linkedin} target="_blank" className="hover:text-zinc-200 transition-colors">
            LinkedIn
          </a>
          {dev.website && (
            <a href={dev.website} target="_blank" className="hover:text-zinc-200 transition-colors">
              Website
            </a>
          )}
        </div>
...
export default function MeetTheDevs() {
  ...
  return (
    <section className="w-full h-full bg-zinc-950 text-zinc-100 py-6 px-4 flex flex-col items-center gap-y-5 gap-x-10 rounded-md">
      <h1 className="text-2xl font-bold text-center underline">Meet the Developers</h1>
